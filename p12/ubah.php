...
$id = $_GET["id"];
$mhs = query("SELECT * FROM db_mhs WHERE id = $id")[0];

// include_once("functions.php");
if(isset($_POST["submit"]) ) {
    

    // cek apakah data berhasil diubahkan
    if (ubah($_POST) > 0) {
        echo "
            <script>
...
<!-- 
                    ...
                    <div class="col-lg-10">
                        <div class="p-5">
                            <div class="text-center">
                                <h1 class="h4 text-gray-900 mb-4">ubah data mahasiswa!</h1>
                            </div>
    ...
    <form class="user" action="" method="post">
        ...
        <div class="form-group">
            ...
            id="nama" placeholder="Nama Lengkap" value="<?= $mhs["nama"] ?>">
        </div>
        <div class="form-group">
            <input type="text" class="form-control form-control-user" name="nim"
            id="nim" placeholder="NIM" required value="<?= $mhs["nim"] ?>">
        </div>
        <div class="form-group">
            <input type="text" class="form-control form-control-user" name="jurusan"
            id="jurusan" placeholder="Jurusan" value="<?= $mhs["jurusan"] ?>">
        </div>
        <button type="submit" name="submit" class="btn btn-success btn-user btn-block">
            Ubah!
        </button>
